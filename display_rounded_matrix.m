...
#Adequate number of them
  if (nargin < 2)
	error("display_rounded_matrix: Wrong number of parameters, at least matrix and significative must be provided.");
  endif;
#Significative values are a vector:
  if (rows(significative)!=1)
	disp("significative:");
	disp(significative);
	error("display_rounded_matrix: Significative vector not a vector");
  endif;
